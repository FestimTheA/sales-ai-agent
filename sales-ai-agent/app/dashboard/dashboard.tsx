...
const MyResponsiveFunnel = ({ data }: { data: FunnelData[] }) => (
  ...
  <div className="relative" style={{ height: "64vh" }}>
    ...
    <div
      className="absolute bottom-0 left-0 right-0 flex gap-4 justify-center"
      style={{ fontSize: "var(--nextui-font-size-small)" }}
    >
    ...
    const transformData = () => {
      ...
      Object.entries(data.steps).forEach(([stepId, step]) => {
        ...
        Object.entries(data.metrics).forEach(([campaignId, campaignData]) => {
          ...
          if (
            campaignFilter === "all" ||
            (data.campaigns as any)[campaignId].name === campaignFilter
          ) {
            ...
            Object.entries(campaignData).forEach(([userId, userData]) => {
              ...
              if (
                userFilter === "all" ||
                (data.users as any)[userId].name === userFilter
              ) {
